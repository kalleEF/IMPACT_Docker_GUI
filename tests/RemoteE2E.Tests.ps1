...
BeforeAll {
    ...
    # Preflight state (do not throw in BeforeAll; expose via an explicit It)
...
Describe 'RemoteE2E: Windows -> SSH -> Workstation -> Docker build/run' -Tag RemoteE2E {
    ...
    BeforeAll {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            Write-Host "[RemoteE2E][Preflight] Docker CLI not available on workstation: $($dockerVer -join ' ')" -ForegroundColor Red
            return
        }
        ...
        if ($checkClone -notmatch 'CLONE_OK') {
            ...
            Write-Host "[RemoteE2E][Preflight] Repo clone failed on workstation: $($cloneOut -join ' ')" -ForegroundColor Red
            return
        }
        ...
        if (-not $script:skipBuild) {
            ...
            if ($LASTEXITCODE -ne 0 -or -not $imgCheck) {
                ...
                if (-not $imgCheck2) {
                    ...
                    Write-Host "[RemoteE2E][Preflight] Docker image build failed on workstation." -ForegroundColor Red
                    return
                }
            }
